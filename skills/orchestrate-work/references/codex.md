...
- Use the collaboration subagent tools for workers belonging to the current task. Give each worker the skill's relevant engineering reference and its concrete assignment. A child working in another Git worktree still needs the absolute worktree path in its assignment; delegation alone does not isolate files.
...
- Use `exec_command` for Git worktree operations and repository checks. Pin the base commit before creating worker branches. Keep a single writer for integration.
...
- Keep the work record in a shared local directory outside worker worktrees. Give workers its absolute path and require them to send proposed updates to the coordinator.
